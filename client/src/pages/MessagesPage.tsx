...
interface Message {
  ...
  content: string;
  shared_with_professor: boolean | number;
  created_at: string;
}
...
export default function MessagesPage() {
  ...
  const navState = (location.state as MessagePageNavState | null) || null;
  ...
  const hasInitializedFromRedirect = useRef(false);
  ...
  useEffect(() => {
    async function init() {
      ...
      if (user?.role === "student") {
        ...
        if (navState?.courseId && !hasInitializedFromRedirect.current) {
          hasInitializedFromRedirect.current = true;

          const result = await createConversation(navState.courseId);
          setSelectedConversationId(result.conversationId);
          await refreshMessages(result.conversationId);

          if (navState.prefill) {
            setText(navState.prefill);
          } else if (navState.slideTitle || navState.slideContent) {
            setText(
              `I need help with "${navState.slideTitle || "this slide"}".\n\n${
                navState.slideContent || ""
              }`
            );
          }
      ...
      if (!selectedConversationId && existingConversations.length > 0) {
  ...
  async function handleCreateConversation(courseId: number) {
    ...
    if (navState?.courseId === courseId && navState?.prefill) {
      setText(navState.prefill);
    } else {
      setText("");
    }
              ...
              >
                ...
                {courses.map((course, i) => (
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize: 13, color: "#3d3d3a" }}>
                      {course.title}
                    </span>
                ...
                conversations.map((conv, i) => {
                  ...
                  return (
                    <button
                      ...
                      onMouseEnter={(e) => {
                        if (!isSelected) e.currentTarget.style.background = "#F8F7FF";
                      }}
                      onMouseLeave={(e) => {
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            color: isSelected ? "#3C3489" : "#1a1a1a",
                          }}
                        >
                          {conv.course_title}
                        </div>
            ...
            {!selectedConversationId ? (
              <div style={{ textAlign: "center", paddingTop: 60, color: "#B4B2A9" }}>
                ...
                <div style={{ fontSize: 13 }}>
                  Choose a course conversation from the left
                </div>
              </div>
            ) : messages.length === 0 ? (
              <div style={{ textAlign: "center", paddingTop: 60, color: "#B4B2A9" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {messages.map((message) => {
                  ...
                  const isShared = Boolean(message.shared_with_professor);
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {user?.role === "student" && isAI && !isShared && (
                          <div>
                            {!isSharingThis ? (
                              <button
          ...
          {selectedConversationId && (
            <div style={{ padding: "12px 20px", borderTop: "0.5px solid #F1EFE8" }}>
              <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 500,
                    background:
                      loadingSend || !text.trim() ? "#AFA9EC" : "#534AB7",
                    color: "#fff",
                    border: "none",
                    borderRadius: 8,
                    cursor:
                      loadingSend || !text.trim() ? "not-allowed" : "pointer",
                    fontFamily: "inherit",
                    flexShrink: 0,
                  }}
                >
              ...
              <div style={{ fontSize: 11, color: "#B4B2A9", marginTop: 6 }}>
                ...
                {user?.role === "student" &&
                  " · AI responses can be shared with your professor"}
              </div>
            </div>
          )}
        </div>
